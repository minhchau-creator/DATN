...
TRAIN_SCRIPT_NAMES      = ["train_models.py", "train_stage2.py", "finetune.py"]
GPU_INDEX               = 0
# ─────────────────────────────────────────────────────────────────────────────
...
def find_datn_pid() -> int | None:
    """Tìm PID của tiến trình DATN training đang chạy (train_models/train_stage2/finetune)."""
    if _PSUTIL:
        for proc in psutil.process_iter(["pid", "cmdline"]):
            try:
                cmdline = " ".join(proc.info["cmdline"] or [])
                if any(name in cmdline for name in TRAIN_SCRIPT_NAMES):
                    return proc.info["pid"]
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                continue
        return None

    for name in TRAIN_SCRIPT_NAMES:
        result = subprocess.run(
            ["pgrep", "-f", name],
